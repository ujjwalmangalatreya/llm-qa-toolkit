...
This is **Phase 1** of a multi-phase build. Each phase adds a new dimension of LLM quality:

| Phase | Theme                                  | Status      |
| ----- | -------------------------------------- | ----------- |
| 1     | Scaffold + smoke tests vs. Gemini      | ✅ shipped   |
| 2     | `docsbot-demo` system under test       | 🛠 building |
...
Future months: bias testing, multimodal QA, agent testing.

---

## Configuration

All configuration goes through `.env.local`. See [.env.example](.env.example) for the full list.

| Variable                 | Purpose                                                                       |
| ------------------------ | ----------------------------------------------------------------------------- |
| `GEMINI_API_KEY`         | Required. Your Gemini API key. Get one free at aistudio.google.com/apikey.    |
| `GEMINI_MODEL`           | Default text model. CI uses `gemini-2.5-flash-lite` for free-tier headroom.   |
| `GEMINI_EMBEDDING_MODEL` | Default embedding model (`gemini-embedding-001`).                             |
| `SIMILARITY_THRESHOLD`   | Default cosine-similarity bar for the matcher (0.75).                         |
| `DOCSBOT_URL`            | URL of the system under test (lands in Phase 2).                              |

---

## Running in CI

The included GitHub Actions workflow runs the smoke suite on every PR. Add `GEMINI_API_KEY` to your repo's **Settings → Secrets and variables → Actions** before the first run. The workflow uploads the Playwright HTML report as an artifact, so every run leaves an inspectable trail behind.

---

## Why provider-neutral naming

The repo is `llm-qa-toolkit` rather than `gemini-qa-toolkit` on purpose. The architecture is deliberately Gemini-first today, but the matcher, similarity utility, and test categories don't care which model is on the other end of the wire. Adding a Claude or OpenAI client later means dropping in a new file under `src/clients/` — not renaming the repo.
